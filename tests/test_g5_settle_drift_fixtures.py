...
def test_override_mjcf_has_inertial(asset_name, expected_mass, tol_frac):
    ...
    delta = abs(mass - expected_mass)
    tol = tol_frac * expected_mass
    assert delta <= tol, f"{asset_name}: mass {mass} outside ±{tol_frac:.0%} of {expected_mass}"
...
def test_patched_fixture_xy_drift_under_settle(asset_name, yaw_idx):
    ...
    try:
        ...
        drift_mm = drift * 1000
        msg = f"{asset_name} yaw={yaw:.2f}: xy drift {drift_mm:.1f} mm exceeds 10 mm"
        assert drift < 0.01, msg
    finally:

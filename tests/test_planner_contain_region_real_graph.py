...
_TRAY = _BDDL_ROOT / "libero_90" / "LIVING_ROOM_SCENE4_pick_up_the_chocolate_pudding_and_put_it_in_the_tray.bddl"
_CADDY = _BDDL_ROOT / "libero_90" / "STUDY_SCENE3_pick_up_the_book_and_place_it_in_the_front_compartment_of_the_caddy.bddl"
...
def test_basket_real_graph_filters_non_container_variants() -> None:
    ...
    for v in pool:
        assert contain_region_sites(v) >= contain_region_sites("basket"), (
            f"variant {v!r} missing required contain_region sites"
        )
...
def test_tray_real_graph_filters_non_container_variants() -> None:
    ...
    assert "chefmate_8_frypan" not in pool, pool
    for v in pool:
        assert contain_region_sites(v) >= contain_region_sites("wooden_tray"), (
            f"variant {v!r} missing required contain_region sites"
        )

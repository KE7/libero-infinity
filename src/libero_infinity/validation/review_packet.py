...
The generator is defensive: when an optional step (rendering, baseline compile)
fails it writes a placeholder file and records the traceback in
``compiler_diagnostics.txt`` so the packet is still complete and inspectable.
"""
...
def _render_scene_png(scenic_src: str, out_path: pathlib.Path) -> str | None:
    """Best-effort: compile + sample once + dump a PNG; return error string or None."""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--subset", default="none", help="Comma-separated axes, preset, or 'none'.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = generate_packet(args.task, args.subset, args.seed, args.out, render=not args.no_render)
